...
DevModeDlg::DevModeDlg(Multistage2015 *_Ms15)
{
	...
	hDlg=NULL;
	char buff[MAX_PATH];
	strcpy(buff,Ms15->OrbiterRoot);
	char slash[5];
	strcpy(slash,"\\");
	//strcat(buff,"\\output.txt");
	strcat(buff,slash);
	strcat(buff,Ms15->fileini);
	WriteIni.SetFilename(buff);
	strcpy(CompleteFileIniName,buff);
...
double DevModeDlg::GetDlgItemDouble(HWND hDlg, int Idd_ctrl ,  int cchMax)
{
	char *buff=new char[cchMax];
	double out;
	GetDlgItemText(hDlg,Idd_ctrl,buff,cchMax);
	sscanf(buff,"%lf",&out);
	delete[] buff;
	return out;
}
VECTOR3 DevModeDlg::GetDlgItemVector3(HWND hDlg, int Idd_ctrlx, int Idd_ctrly, int Idd_ctrlz, int cchMax)
{
	VECTOR3 out;
	char *buff=new char[cchMax];
	GetDlgItemText(hDlg,Idd_ctrlx,buff,cchMax);
	sscanf(buff,"%lf",&out.x);
	GetDlgItemText(hDlg,Idd_ctrly,buff,cchMax);
	sscanf(buff,"%lf",&out.y);
	GetDlgItemText(hDlg,Idd_ctrlz,buff,cchMax);
	sscanf(buff,"%lf",&out.z);
	delete[] buff;
	return out;
}
VECTOR4F DevModeDlg::GetDlgItemVector4(HWND hDlg, int Idd_ctrlx, int Idd_ctrly, int Idd_ctrlz, int Idd_ctrlt, int cchMax)
{
	...
	GetDlgItemText(hDlg,Idd_ctrlz,buff,cchMax);
	sscanf(buff,"%lf",&out.z);
	GetDlgItemText(hDlg,Idd_ctrlt,buff,cchMax);
	sscanf(buff,"%lf",&out.t);
	delete[] buff;
	return out;

}
void DevModeDlg::RefreshGraph (Graph *graph, int GraphId)
{
	
	HWND hCtrl = GetDlgItem (hChild[CD_CURVE], GraphId);
	InvalidateRect (hCtrl, NULL, TRUE);
	InvalidateRect( hChild[CD_CURVE],NULL,TRUE);
	UpdateWindow (hCtrl);
//	UpdateWindow(hChild[CD_CURVE]);
	RECT rect;
	HDC hDC = GetDC (hCtrl);
...
void DevModeDlg::InitDialog(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,bool InitPrms=TRUE, ItemType Expand=Empty)
{
		...
		/*insertstruct.item.pszText=(LPSTR)TEXT("Interstages\0");
		insertstruct.item.cchTextMax=12;
		HTREEITEM hrootInterstages = TreeView_InsertItem(GetDlgItem(hWnd,IDC_TREE1),&insertstruct);*/
		...
		/*for(UINT i=0;i<DMD_nInterstages;i++)
		{
			insertstruct.hParent=hrootInterstages;
			char text[256];
			sprintf(text,"Interstage %i\0",i+1);
			insertstruct.item.pszText=(LPSTR)text;
			insertstruct.item.cchTextMax=14;
			TreeView_InsertItem(GetDlgItem(hWnd,IDC_TREE1),&insertstruct);
		}*/
		...
		{
			insertstruct.hParent=hrootTextures;
			char text[256];
			sprintf(text,"Textures %i\0",i+1);
			insertstruct.item.pszText=(LPSTR)text;
			insertstruct.item.cchTextMax=12;
			TreeView_InsertItem(GetDlgItem(hWnd,IDC_TREE1),&insertstruct);
		}*/
		...
		//FAIRING HIDDEN RESTORATION
			...
			UINT half=Ms15->fairing.N;
				half+=1;
				half=half/2;
						
				if(hideFairing){
							for(UINT NF=1;NF<Ms15->fairing.N+1;NF++)
							{
							Ms15->SetMeshVisibilityMode(Ms15->fairing.msh_idh[NF],MESHVIS_NEVER);
							}
...
{
//Multistage2015 *Ms15 = (uMsg == WM_INITDIALOG ? (Multistage2015*)lParam : (Multistage2015*)oapiGetDialogContext (hWnd));	
	...
	switch (uMsg) {
		case WM_INITDIALOG: 
			{
				...
				sprintf(DestFile,"%s\\Config\\Multistage2015\\Backups\\%s_Backup_%.4f.ini",Ms15->OrbiterRoot,Ms15->GetName(),oapiGetSysMJD());
				bool backupFile=CopyFile((LPCSTR)CompleteFileIniName,(LPCSTR)DestFile,FALSE);
				if(backupFile){
		...
		case IDCANCEL:
			...
			Ms15->SendBufferedKey(OAPI_KEY_SPACE,true,kstate);
		...
		case IDC_VIEWINI:
			{
				...
				if(inifile.is_open())
				{
					while (getline(inifile,line))
					{
						...
						while((pos2=line.find("\r",pos2))!=string::npos)
						{
							line.erase(pos2,1);
						}
		...
		case IDC_SAVEINI:
			{
				...
				if(inifile.is_open()){
					string str(buff);
					string::size_type pos2=0;
						while((pos2=str.find("\r",pos2))!=string::npos)
						{
							str.erase(pos2,1);
						}
		...
		case IDC_RESET:
			Ms15->ResetVehicle(Ms15->hangaranims,Ms15->wRamp);
...
bool DevModeDlg::SaveScenarioFile(bool defname)
{
	...
	if(defname)
	{
	sprintf(filebuff,"Multistage2015\\%s - %.4f",Ms15->GetName(),oapiGetSysMJD());
	}else{
		char filebuffname[128];
	GetDlgItemText(hChild[CD_SCENARIO],IDC_SCN_FILENAME,filebuffname,128);
	sprintf(filebuff,"Multistage2015\\");
...
BOOL DevModeDlg::ScenarioProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		{
			SetDlgItemText(hChild[CD_SCENARIO],IDC_SCN_SHIPNAME,(LPCSTR)Ms15->GetName());
			...
			SetDlgItemText(hChild[CD_SCENARIO],IDC_SCN_CONFIGFILE,(LPCSTR)Ms15->fileini);
	...
	case WM_COMMAND:
		{
			...
			/*case IDC_SCN_SAVELOADSCN:
				{
					getcheck=SendDlgItemMessage(hChild[CD_SCENARIO],IDC_SCN_FNDEFAULT,BM_GETCHECK,0,0);
					if(getcheck==BST_CHECKED)
					{
						SaveScenarioFile(TRUE,TRUE);
					}else{
						SaveScenarioFile(FALSE,TRUE);
					}
					break;
				}*/
			}
			...
			case IDC_FX_STOREVALS:
				{
					...
					{
						DMD_wFXLaunch=TRUE;
						GetDlgItemText(hChild[CD_FX],IDC_FXLAUNCH_PS1,DMD_FX_Launch.Ps1,128);
						GetDlgItemText(hChild[CD_FX],IDC_FXLAUNCH_PS2,DMD_FX_Launch.Ps2,128);
		...
		case IDC_TEX_STOREVALS:
			{
				...
				{
					buff[0]='\0';
					GetDlgItemText(hChild[CD_TEX],IDC_TEX_TEX1+i,buff,MAXLEN);
					if(buff[0]!='\0')
					{
						for(UINT j=0;j<MAXLEN;j++)
						{
							DMD_tex.TextureName[j][i]=buff[j];
						}
					}else{
					DMD_nTextures=i;
					break;
		...
		case IDC_MISC_STOREVALS:
			{
				...
				if(GetCheck==BST_CHECKED)
				{
					GetDlgItemText(hChild[CD_MISC],IDC_MISC_PADMODULE,DMD_Misc.PadModule,MAXLEN);
				}else{
					sprintf(DMD_Misc.PadModule,"EmptyModule");
				}
			...
			case IDC_PART_TESTPART:
				{
					...
					test_part_h=Ms15->AddParticleStream(&test_particle,_V(75,0,10),_V(0,0,-1),&sliderlvl);
					...
					*/
						...
						TestingParticle=FALSE;
						Ms15->DelExhaustStream(test_part_h);
				...
				case IDC_PART_STOREVALS:
					...
					GetDlgItemText(hChild[CD_PARTICLE],IDC_PART_NAME,DMD_Particle[partid].ParticleName,MAXLEN);					
					...
					sprintf(DMD_Particle[partid].TexName,"%s", buff);
			...
			case IDC_FAIR_HIDE:
				{
					if(HIWORD(wParam)== BN_CLICKED)
					{
						...
						if(getcheck==BST_CHECKED){
							...
							for(UINT NF=1;NF<Ms15->fairing.N+1;NF++)
							{
							Ms15->SetMeshVisibilityMode(Ms15->fairing.msh_idh[NF],MESHVIS_EXTERNAL);
							hideFairing=FALSE;
							}
						}
					}
...
BOOL DevModeDlg::PldProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	/*	case IDC_PLD_UPDMSH:
			{
				pldid=GetDlgItemInt(hChild[CD_PLD],IDC_PLD_ID,FALSE,FALSE);
				for(int i=0;i<Ms15->payload[pldid].nMeshes;i++)
				{
				Ms15->ShiftMesh(Ms15->payload[pldid].msh_idh[i],-);
				}
				break;
			}*/
...
void DevModeDlg::InitParams()
{
	for(int i=0;i<10;i++)
	{
		DMD_stage[i]=Ms15->stage[i];
		DMD_booster[i]=Ms15->booster[i];
		DMD_payload[i]=Ms15->payload[i];
	}
	DMD_fairing=Ms15->fairing;
	DMD_Misc=Ms15->Misc;
	DMD_tex=Ms15->tex;
	DMD_Les=Ms15->Les;
	...
	for(int j=0;j<16;j++)
	{
	DMD_Particle[j]=Ms15->Particle[j];
	}

	DMD_nStages=Ms15->nStages;
	DMD_nBoosters=Ms15->nBoosters;
	DMD_nInterstages=Ms15->nInterstages;
	DMD_nPayloads=Ms15->nPayloads;
	DMD_nParticles=Ms15->nParticles;
	DMD_nTextures=Ms15->nTextures;
	DMD_wFairing=Ms15->wFairing;
	DMD_wAdapter=Ms15->wAdapter;
	DMD_wFXLaunch=Ms15->wLaunchFX;
	if(DMD_wAdapter)
	{
		DMD_Adapter=Ms15->Adapter;
	}else{
		DMD_Adapter=ADAPTER();
	}
	DMD_wLes=Ms15->wLes;
	DMD_wMach=Ms15->wMach;
	DMD_wVent=Ms15->wVent;
	DMD_FX_Vent=Ms15->FX_Vent;
	DMD_FX_Mach=Ms15->FX_Mach;
	DMD_FX_Launch=Ms15->FX_Launch;
	ReverseOffsets();
}
...
void DevModeDlg::PopulateStages(UINT iddialog, UINT id)
{
	...
	string engx[5],engy[5],engz[5],engt[5];
...
void DevModeDlg::SetMultipleDlgItemsText(HWND hDlg, int *idc, string *str, UINT nInputs)
{
...
void DevModeDlg::PopulateChildDialog(UINT iddialog, UINT id)
{
	...
	if(DMD_payload[id-1].nMeshes>1)
	{
		...
		string meshname(DMD_payload[id-1].meshname);
		char quotes[1]={'\"'};
	...
	case CD_PARTICLE:
		{
			...
			SetDlgItemText(hChild[iddialog],IDC_PART_NAME,DMD_Particle[id-1].ParticleName);
			...
			*/
			...
			SetDlgItemText(hChild[iddialog],IDC_PART_TEX,DMD_Particle[id-1].TexName);
		...
		case CD_MISC:
		{
			...
			if(strncmp(DMD_Misc.PadModule,"EmptyModule",11)==0)
			{
				...
				SetDlgItemText(hChild[iddialog],IDC_MISC_PADMODULE,(LPCSTR)DMD_Misc.PadModule);
		...
		case CD_TEX:
		{
			for(UINT i=0;i<DMD_nTextures;i++)
			{
				...
				for(UINT j=0;j<MAXLEN;j++)
				{
					trans[j]=DMD_tex.TextureName[j][i];
				}
				SetDlgItemText(hChild[iddialog],IDC_TEX_TEX1+i,trans);
			}
			break;
		}
		case CD_BOOSTER:
		...
		case CD_FX:
		{
			...
			if(DMD_wVent)
			{
				...
				for(UINT i=0;i<DMD_FX_Vent.nVent;i++)
				{
					...
					VentTxt[7*i+counter]=string(buff);
					counter++;
			...
			if(DMD_wMach)
			{
				...
				for(UINT q=0;q<DMD_FX_Mach.nmach;q++)
				{
					...
					MachTxt[q*3+counter]=string(buff);
					counter++;
			...
			if(DMD_wFXLaunch)
			{
				...
				SetDlgItemText(hChild[CD_FX],IDC_FXLAUNCH_PS1,DMD_FX_Launch.Ps1);
				SetDlgItemText(hChild[CD_FX],IDC_FXLAUNCH_PS2,DMD_FX_Launch.Ps2);
			}else{
	...
	case Payload:
		{
			for(UINT i=0;i<DMD_nPayloads;i++)
			{
				...
				strcpy(DMD_payload[i].MultiOffset,buffer);
...
void DevModeDlg::Notify(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
 ...
 {
	double currentDelta=Ms15->currentDelta;
	
	int i;
	for(i=Ms15->currentStage;i<DMD_nStages;i++){
	...
	int p;
	for(p=Ms15->currentPayload;p<DMD_nPayloads;p++){
	...
	int z;
	for(z=Ms15->currentBooster;z<DMD_nBoosters;z++){
 ...
 /*
void DevModeDlg::ArrangePayloadMeshes(char data[MAXLEN], int pnl){
	string meshnm(data);
	...
	std::size_t findFirstSC=meshnm.find_first_of(";");
		...
		meshnm0.copy(DMD_payload[pnl].meshname0,MAXLEN,0);
				...
				if(findThirdSC!=meshnm.npos){
					meshnm2=meshnm.substr(findSecondSC+1,findThirdSC-findSecondSC-1);
						...
						if(findFourthSC!=meshnm.npos){
							...
							DMD_payload[pnl].nMeshes=5;
						}else{  meshnm3=meshnm.substr(findThirdSC+1,meshnm.npos);DMD_payload[pnl].nMeshes=4;}
				}else{meshnm2=meshnm.substr(findSecondSC+1,meshnm.npos);DMD_payload[pnl].nMeshes=3;}
			}else{meshnm1=meshnm.substr(findFirstSC+1,meshnm.npos);DMD_payload[pnl].nMeshes=2;}
		}else{meshnm0=meshnm.substr(0,meshnm.npos);DMD_payload[pnl].nMeshes=1;}
		
		meshnm0.copy(DMD_payload[pnl].meshname0,MAXLEN,0);
	//	oapiWriteLog(payload[pnl].meshname0);
		if(DMD_payload[pnl].nMeshes==5){
		meshnm1.copy(DMD_payload[pnl].meshname1,MAXLEN,0);
		meshnm2.copy(DMD_payload[pnl].meshname2,MAXLEN,0);
		meshnm3.copy(DMD_payload[pnl].meshname3,MAXLEN,0);
		meshnm4.copy(DMD_payload[pnl].meshname4,MAXLEN,0);
		}else if(DMD_payload[pnl].nMeshes==4){
		meshnm1.copy(DMD_payload[pnl].meshname1,MAXLEN,0);
		meshnm2.copy(DMD_payload[pnl].meshname2,MAXLEN,0);
		meshnm3.copy(DMD_payload[pnl].meshname3,MAXLEN,0);
		}else if(DMD_payload[pnl].nMeshes==3){
		meshnm1.copy(DMD_payload[pnl].meshname1,MAXLEN,0);
		meshnm2.copy(DMD_payload[pnl].meshname2,MAXLEN,0);
		}else if(DMD_payload[pnl].nMeshes==2){
		meshnm1.copy(DMD_payload[pnl].meshname1,MAXLEN,0);
		}
		 ...
		 Ms15->CharToVec(coff1,&DMD_payload[pnl].off[1]);
				...
				if(DMD_payload[pnl].nMeshes==3){
					...
					if(DMD_payload[pnl].nMeshes==4){
						...
						Ms15->CharToVec(coff4,&DMD_payload[pnl].off[4]);
					}
				}
		 }
	 }
