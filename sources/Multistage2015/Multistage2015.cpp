...
Multistage2015::Multistage2015(OBJHANDLE hObj,int fmodel):VESSEL3(hObj,fmodel){
	...
	for (unsigned int i = 0; i < 10; i++) {
		booster[i].Ignited = false;
		booster[i].ParticlesPacked = false;
		booster[i].ParticlesPackedToEngine = 0;
	}
 ...
 for(i=0;i<stage[currentStage].nEngines;i++){
  ...
  if(!stage[currentStage].ParticlesPacked){
	  if(stage[currentStage].wps1){
		PARTICLESTREAMSPEC Pss1 = GetProperPS(stage[currentStage].eng_pstream1).Pss;
		AddExhaustStreamGrowing(stage[currentStage].th_main_h[i],stage[currentStage].eng[i],&Pss1,GetProperPS(stage[currentStage].eng_pstream1).Growing,GetProperPS(stage[currentStage].eng_pstream1).GrowFactor_size,GetProperPS(stage[currentStage].eng_pstream1).GrowFactor_rate,TRUE,FALSE,currentStage,i);
	...
	if(stage[currentStage].wps2){
		PARTICLESTREAMSPEC Pss2 = GetProperPS(stage[currentStage].eng_pstream2).Pss;
	  AddExhaustStreamGrowing(stage[currentStage].th_main_h[i],stage[currentStage].eng[i],&Pss2,GetProperPS(stage[currentStage].eng_pstream2).Growing,GetProperPS(stage[currentStage].eng_pstream2).GrowFactor_size,GetProperPS(stage[currentStage].eng_pstream2).GrowFactor_rate,TRUE,FALSE,currentStage,i);
...
*/
	...
	{
		PARTICLESTREAMSPEC Pss3 = GetProperPS(stage[currentStage].expbolt.pstream).Pss;
		stage[currentStage].expbolt.threxp_h=CreateThruster(stage[currentStage].expbolt.pos,stage[currentStage].expbolt.dir,0,stage[currentStage].tank,100000,100000);
		AddExhaustStream(stage[currentStage].expbolt.threxp_h,&Pss3);
	}
...
//	psg[ps].Th_idx=ps;
 ...
 for(bi=currentBooster;bi<nBoosters;bi++){
	...
	if(booster[bi].nEngines==0){
	 ...
	 if(booster[bi].wps1){
		PARTICLESTREAMSPEC Pss4 = GetProperPS(booster[bi].eng_pstream1).Pss;
	AddExhaustStreamGrowing(booster[bi].th_booster_h[bii],engofs,&Pss4,GetProperPS(booster[bi].eng_pstream1).Growing,GetProperPS(booster[bi].eng_pstream1).GrowFactor_size,GetProperPS(booster[bi].eng_pstream1).GrowFactor_rate,TRUE,TRUE,bi,bii);
	...
	oapiWriteLog(logbuff);}
	 if(booster[bi].wps2){
		PARTICLESTREAMSPEC Pss5 = GetProperPS(booster[bi].eng_pstream2).Pss;
	 AddExhaustStreamGrowing(booster[bi].th_booster_h[bii],engofs,&Pss5,GetProperPS(booster[bi].eng_pstream2).Growing,GetProperPS(booster[bi].eng_pstream2).GrowFactor_size,GetProperPS(booster[bi].eng_pstream2).GrowFactor_rate,TRUE,TRUE,bi,bii);
 ...
 for(bii=0;bii<booster[bi].nEngines;bii++){
	for(biii=1;biii<booster[bi].N+1;biii++){
	 ...
	 if(booster[bi].wps1){
		PARTICLESTREAMSPEC Pss6 = GetProperPS(booster[bi].eng_pstream1).Pss;
	 AddExhaustStreamGrowing(booster[bi].th_booster_h[biii-1],engofs,&Pss6,GetProperPS(booster[bi].eng_pstream1).Growing,GetProperPS(booster[bi].eng_pstream1).GrowFactor_size,GetProperPS(booster[bi].eng_pstream1).GrowFactor_rate,TRUE,TRUE,bi,biii-1);
	 ...
	 oapiWriteLog(logbuff);}
	 if(booster[bi].wps2){
		PARTICLESTREAMSPEC Pss7 = GetProperPS(booster[bi].eng_pstream2).Pss;
	 AddExhaustStreamGrowing(booster[bi].th_booster_h[biii-1],engofs,&Pss7,GetProperPS(booster[bi].eng_pstream2).Growing,GetProperPS(booster[bi].eng_pstream2).GrowFactor_size,GetProperPS(booster[bi].eng_pstream2).GrowFactor_rate,TRUE,TRUE,bi,biii-1);
	 ...
	 oapiWriteLog(logbuff);
		...
		{
			booster[bi].expbolt.threxp_h=CreateThruster(booster[bi].expbolt.pos,booster[bi].expbolt.dir,0,booster[bi].tank,100000,100000);
			PARTICLESTREAMSPEC Pss8 = GetProperPS(booster[bi].expbolt.pstream).Pss;
			AddExhaustStream(booster[bi].expbolt.threxp_h,&Pss8);
		}
