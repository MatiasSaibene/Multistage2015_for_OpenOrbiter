...
Multistage2015::Multistage2015(OBJHANDLE hObj, int fmodel) :VESSEL3(hObj, fmodel) {
	...
	for (int i = 0; i < 7200; i++) {
		ReftlmAcc[i] = VECTOR2(0, 0);
	}

	for (int i = 0; i < 7200; i++) {
		ReftlmAlt[i] = VECTOR2(0, 0);
	}

	for (int i = 0; i < 7200; i++) {
		ReftlmMass[i] = VECTOR2(0, 0);
	}

	for (int i = 0; i < 7200; i++) {
		ReftlmSpeed[i] = VECTOR2(0, 0);
	}

	for (int i = 0; i < 7200; i++) {
		ReftlmPitch[i] = VECTOR2(0, 0);
	}

	for (int i = 0; i < 7200; i++) {
		ReftlmThrust[i] = VECTOR2(0, 0);
	}

	for (int i = 0; i < 7200; i++) {
		ReftlmVv[i] = VECTOR2(0, 0);
	}
...
VECTOR3 Multistage2015::CharToVec(const std::string& str) {
...
VECTOR4F Multistage2015::CharToVec4(const std::string &str) {
    std::string s = str;
    s.erase(std::remove(s.begin(), s.end(), '('), s.end());
    s.erase(std::remove(s.begin(), s.end(), ')'), s.end());
    std::replace(s.begin(), s.end(), ';', ',');
...
void Multistage2015::CreateMainThruster() {
	...
	if (Misc.thrustrealpos) {
		...
		//stage.at(currentStage).th_main_h[0]=CreateThruster(stage.at(currentStage).off,stage.at(currentStage).eng_dir,stage.at(currentStage).thrust,stage.at(currentStage).tank,stage.at(currentStage).isp);
	}
	else {
		for (int i = 0; i < stage.at(currentStage).nEngines; i++) {
	...
	//-------------------------------------------------------------------------------------------------------	
		...
		if(Misc.thrustrealpos){
			for(int i=0;i<stage.at(currentStage).nEngines;i++){
				stage.at(currentStage).th_main_h.at(i)=CreateThruster(stage.at(currentStage).off,stage.at(currentStage).eng_dir,stage.at(currentStage).thrust/stage.at(currentStage).nEngines,stage.at(currentStage).tank,stage.at(currentStage).isp);
			}
			//stage.at(currentStage).th_main_h[0]=CreateThruster(stage.at(currentStage).off,stage.at(currentStage).eng_dir,stage.at(currentStage).thrust,stage.at(currentStage).tank,stage.at(currentStage).isp);
		}else{
			for(int i=0;i<stage.at(currentStage).nEngines;i++){
				stage.at(currentStage).th_main_h.at(i)=CreateThruster(_V(0,0,0),_V(0,0,1),stage.at(currentStage).thrust/stage.at(currentStage).nEngines,stage.at(currentStage).tank,stage.at(currentStage).isp);
					}
		}*/
		//stage.at(currentStage).th_main_h[0]=CreateThruster(_V(0,0,0),_V(0,0,1),stage.at(currentStage).thrust,stage.at(currentStage).tank,stage.at(currentStage).isp);}
 //thg_h_main=CreateThrusterGroup(stage.at(currentStage).th_main_h,1,THGROUP_MAIN);
	...
	for (i = 0; i < stage.at(currentStage).nEngines; i++) {
		...
		exhaustN[currentStage][i] = AddExhaust(stage.at(currentStage).th_main_h.at(i), 10 * stage.at(currentStage).eng_diameter * stage.at(currentStage).engV4[i].t, stage.at(currentStage).eng_diameter * stage.at(currentStage).engV4[i].t, stage.at(currentStage).eng[i], operator*(stage.at(currentStage).eng_dir, -1), GetProperExhaustTexture(stage.at(currentStage).eng_tex.data()));
		...
		if (!stage.at(currentStage).ParticlesPacked) {
			if (stage.at(currentStage).wps1) {
				...
				AddExhaustStreamGrowing(stage.at(currentStage).th_main_h.at(i), stage.at(currentStage).eng[i], &Pss1, GetProperPS(stage.at(currentStage).eng_pstream1.data()).Growing, GetProperPS(stage.at(currentStage).eng_pstream1.data()).GrowFactor_size, GetProperPS(stage.at(currentStage).eng_pstream1.data()).GrowFactor_rate, TRUE, false, currentStage, i);
				sprintf(logbuff, "%s: Stage n.%i Engine Exhaust Stream Added: %s to engine n.%i", GetName(), currentStage + 1, stage.at(currentStage).eng_pstream1.c_str(), i + 1);
				oapiWriteLog(logbuff);
			}
			if (stage.at(currentStage).wps2) {
				PARTICLESTREAMSPEC Pss2 = GetProperPS(stage.at(currentStage).eng_pstream2.data()).Pss;
				AddExhaustStreamGrowing(stage.at(currentStage).th_main_h.at(i), stage.at(currentStage).eng[i], &Pss2, GetProperPS(stage.at(currentStage).eng_pstream2.data()).Growing, GetProperPS(stage.at(currentStage).eng_pstream2.data()).GrowFactor_size, GetProperPS(stage.at(currentStage).eng_pstream2.data()).GrowFactor_rate, TRUE, false, currentStage, i);
				sprintf(logbuff, "%s: Stage n.%i Engine Exhaust Stream Added: %s to engine n.%i", GetName(), currentStage + 1, stage.at(currentStage).eng_pstream2.c_str(), i + 1);
				oapiWriteLog(logbuff);
			}
		}
		sprintf(logbuff, "%s: Stage n. %i Engines Exhaust Added--> number of engines: %i , diameter: %.3f, position x: %.3f y: %.3f z: %.3f", GetName(), currentStage + 1, stage.at(currentStage).nEngines, stage.at(currentStage).eng_diameter * stage.at(currentStage).engV4[i].t, stage.at(currentStage).eng[i].x, stage.at(currentStage).eng[i].y, stage.at(currentStage).eng[i].z);
		oapiWriteLog(logbuff);
		//oapiWriteLogV("Adding 1: %s 2: %s",stage.at(currentStage).eng_pstream1,stage.at(currentStage).eng_pstream2);
	}

	if (stage.at(currentStage).ParticlesPacked)
	{
		PARTICLESTREAMSPEC partpacked[2];
		partpacked[0] = GetProperPS(stage.at(currentStage).eng_pstream1.data()).Pss;
		...
		if (stage.at(currentStage).ParticlesPackedToEngine > 0) {
			/*Pos[0].x=stage.at(currentStage).eng[engine].x+thdir.x*partpacked[0].srcsize*0.5;
			Pos[0].y=stage.at(currentStage).eng[engine].y+thdir.y*partpacked[0].srcsize*0.5;
			Pos[0].z=stage.at(currentStage).eng[engine].z+thdir.z*partpacked[0].srcsize*0.5;

			Pos[1].x=stage.at(currentStage).eng[engine].x+thdir.x*partpacked[1].srcsize*0.5;
			Pos[1].y=stage.at(currentStage).eng[engine].y+thdir.y*partpacked[1].srcsize*0.5;
			Pos[1].z=stage.at(currentStage).eng[engine].z+thdir.z*partpacked[1].srcsize*0.5;*/
		...
		if (stage.at(currentStage).wps1) {
			...
			sprintf(logbuff, "%s: Stage n.%i Engine Packed Exhaust Stream Added: %s to engine n.%i", GetName(), currentStage + 1, stage.at(currentStage).eng_pstream1.c_str(), engine + 1);
			oapiWriteLog(logbuff);
		}
		if (stage.at(currentStage).wps2) {
			AddExhaustStreamGrowing(stage.at(currentStage).th_main_h.at(engine), Pos[1], &partpacked[1], GetProperPS(stage.at(currentStage).eng_pstream2.data()).Growing, GetProperPS(stage.at(currentStage).eng_pstream2.data()).GrowFactor_size, GetProperPS(stage.at(currentStage).eng_pstream2.data()).GrowFactor_rate, TRUE, false, currentStage, engine);
			sprintf(logbuff, "%s: Stage n.%i Engine Packed Exhaust Stream Added: %s to engine n.%i", GetName(), currentStage + 1, stage.at(currentStage).eng_pstream2.c_str(), engine + 1);
			oapiWriteLog(logbuff);
		}
...
char* Multistage2015::GetProperPayloadMeshName(int pnl, int n) {
	if (n == 0) {
		return payload[pnl].meshname0.data();
	}
	else if (n == 1) {
		return payload[pnl].meshname1.data();
	}
	else if (n == 2) {
		return payload[pnl].meshname2.data();
	}
	else if (n == 3) {
		return payload[pnl].meshname3.data();
	}
	else if (n == 4) {
		return payload[pnl].meshname4.data();
	}
	else {
		return payload[pnl].meshname0.data();
	}
...
void Multistage2015::LoadMeshes() {
	...
	for (q = currentStage; q < nStages; q++) {
		...
		stage[q].msh_h = oapiLoadMeshGlobal(stage[q].meshname.c_str());
		sprintf(logbuff, "%s: Stage n.%i Mesh Preloaded: %s", GetName(), q + 1, stage[q].meshname.c_str());
		oapiWriteLog(logbuff);
		stage[q].msh_idh = AddMesh(stage[q].msh_h, &pos);
		sprintf(logbuff, "%s: Stage n.%i Mesh Added Mesh: %s @ x:%.3f y:%.3f z:%.3f", GetName(), q + 1, stage[q].meshname.c_str(), pos.x, pos.y, pos.z);
		oapiWriteLog(logbuff);
		if (stage[q].wInter == TRUE) {

			VECTOR3 inspos = stage[q].interstage.off;
			stage[q].interstage.msh_h = oapiLoadMeshGlobal(stage[q].interstage.meshname.c_str());
			...
			sprintf(logbuff, "%s: Interstage Mesh Added: %s @ x:%.3f y:%.3f z:%.3f", GetName(), stage[q].interstage.meshname.c_str(), inspos.x, inspos.y, inspos.z);
			oapiWriteLog(logbuff);
		}
	}
	...
	if (wAdapter == TRUE) {
		...
		sprintf(logbuff, "%s: Adapter Mesh Added: %s @ x:%.3f y:%.3f z:%.3f", GetName(), Adapter.meshname.c_str(), adappos.x, adappos.y, adappos.z);
		oapiWriteLog(logbuff);
	...
	if (wLes == TRUE)
	{
		...
		sprintf(logbuff, "%s: Les Mesh Added %s @ x:%.3f y:%.3f z:%.3f", GetName(), Les.meshname.c_str(), LesPos.x, LesPos.y, LesPos.z);
		oapiWriteLog(logbuff);
	}
...
void Multistage2015::UpdateLivePayloads() {
	...
	for (int pns = currentPayload; pns < nPayloads; pns++) {
		if (payload[pns].live) {
			...
			vslive.version = 2;
			OBJHANDLE checklive = oapiGetVesselByName(payload[pns].name.data());
...
//double angleprog=0;
		for (int i = 0; i < stage.at(currentStage).ullage.N; i++)
		{
			...
			//VECTOR3 ull_ofs=operator+(GetBoosterPos(bi,biii),RotateVecZ(booster.at(bi).eng[bii],angle));
...
void Multistage2015::VehicleSetup() {
	...
	for (bk = currentBooster; bk < nBoosters; bk++) {
		...
		sprintf(logbuff, "%s: booster n. %i Tank Added: %.3f kg", GetName(), bk + 1, booster.at(bk).fuelmass * booster.at(bk).N);
		oapiWriteLog(logbuff);
	}
	...
	for (bi = currentBooster; bi < nBoosters; bi++) {
		for (int bn = 0; bn < booster.at(bi).N; bn++)
		{
			...
			//booster.at(bi).th_booster_h.at(0)=CreateThruster(pos,dir,booster.at(bi).N*booster.at(bi).thrust,booster.at(bi).tank,booster.at(bi).isp);
		...
		if (booster.at(bi).nEngines == 0) {
			for (bii = 0; bii < booster.at(bi).N; bii++) {
				...
				if (booster.at(bi).wps1) {
					...
					sprintf(logbuff, "%s: Booster Group n.%i Engine Exhaust Stream Added: %s to engine n.%i", GetName(), bi + 1, booster.at(bi).eng_pstream1.c_str(), bii + 1);
					oapiWriteLog(logbuff);
				}
				if (booster.at(bi).wps2) {
					...
					sprintf(logbuff, "%s: Booster Group n.%i Engine Exhaust Stream Added: %s to engine n.%i", GetName(), bi + 1, booster.at(bi).eng_pstream2.c_str(), bii + 1);
					oapiWriteLog(logbuff);
				}

				sprintf(logbuff, "%s: Booster Engines Exhaust Added--> Booster Group: %i number of engines: %i , diameter: %.3f, position x: %.3f y: %.3f z: %.3f", GetName(), bi + 1, booster.at(bi).nEngines, booster.at(bi).eng_diameter, engofs.x, engofs.y, engofs.z);
				oapiWriteLog(logbuff);
			}
		}
		else {
			...
			for (bii = 0; bii < booster.at(bi).nEngines; bii++) {
				for (biii = 1; biii < booster.at(bi).N + 1; biii++) {
					...
					if (booster.at(bi).wps1) {
						...
						sprintf(logbuff, "%s: Booster Group n.%i Engine Exhaust Stream Added: %s to engine n.%i", GetName(), bi + 1, booster.at(bi).eng_pstream1.c_str(), biii);
						oapiWriteLog(logbuff);
					}
					if (booster.at(bi).wps2) {
						...
						sprintf(logbuff, "%s: Booster Group n.%i Engine Exhaust Stream Added: %s to engine n.%i", GetName(), bi + 1, booster.at(bi).eng_pstream2.data(), biii);
						oapiWriteLog(logbuff);
					}
...
void Multistage2015::Spawn(int type, int current) {
	...
	case TBOOSTER:
		...
		for (i = 1; i < booster.at(current).N + 1; i++) {
			...
			//double vro=length(booster.at(current).speed);
			//vel =_V(vro*cos(booster.at(current).angle*RAD+(i-1)*2*PI/booster.at(current).N),vro*sin(booster.at(current).angle*RAD+(i-1)*2*PI/booster.at(current).N),booster.at(current).speed.z);
	...
	case TPAYLOAD:
		if (!payload[current].live) {
			...
			OBJHANDLE hpl;
			hpl = oapiCreateVesselEx(payload[current].name.c_str(), payload[current].module.c_str(), &vs);
		...
		sprintf(logbuff, "%s: Payload n.%i jettisoned name: %s @%.3f", GetName(), current + 1, payload[current].name.c_str(), MET);
		oapiWriteLog(logbuff);
...
void Multistage2015::ComplexFlight()
{
	...
	if (UpdateComplex >= 1)
	{
		...
		for (int i = 0; i < stage.at(currentStage).nEngines; i++)
		{
			...
			/*double test=(1+stage.at(currentStage).engine_amp[i]*sin(2*PI/60*MET+stage.at(currentStage).engine_phase[i]));
...
void Multistage2015::FLY(double simtime, double simdtime, double mjdate) {
	...
	if ((!stage.at(currentStage).Ignited) && (stage.at(currentStage).currDelay <= 0) && (stage.at(currentStage).StageState == STAGE_WAITING)) {
		...
		sprintf(logbuff, "%s Stage n: %i ignited @%.1f", GetName(), currentStage + 1, stage.at(currentStage).IgnitionTime);
		oapiWriteLog(logbuff);
	}
	//MET+=simdtime;
		...
		for (kb = currentBooster; kb < nBoosters; kb++) {
			if (booster.at(kb).currDelay <= 0) {
				if (booster.at(kb).Ignited == false) {
					...
					sprintf(logbuff, "%s Booster n: %i ignited @%.1f", GetName(), kb + 1, booster.at(kb).IgnitionTime);
					oapiWriteLog(logbuff);
				}
				else {
				...
				//SetThrusterLevel(booster.at(kb).th_booster_h.at(0),Level[kb]);
...
void Multistage2015::EvaluateComplexFlight()
{
	...
	for (int i = 0; i < nStages; i++)
	{
		for (int q = 0; q < stage.at(i).nEngines; q++)
		{
			...
			sprintf(logbuff, "%s Complex Flight-> Stage %i Engine %i Amplitude %.3f phase %.3f frequency %.3f", GetName(), i + 1, q + 1, stage.at(i).engine_amp[q], stage.at(i).engine_phase[q], stage.at(i).freq[q]);
			oapiWriteLog(logbuff);
		}
	}
	...
	for (int j = 0; j < nBoosters; j++)
	{
		for (int z = 0; z < booster.at(j).N; z++)
		{
			...
			sprintf(logbuff, "%s Complex Flight-> Booster %i Engine %i Amplitude %.3f phase %.3f frequency %.3f", GetName(), j + 1, z + 1, booster.at(j).engine_amp[z], booster.at(j).engine_phase[z], booster.at(j).freq[z]);
			oapiWriteLog(logbuff);
		}
	}
...
void Multistage2015::Failure()
{
		...
		//if((currentStage==0)&&(check>0))
	{
		...
		//double newlevel=stage.at(currentStage).thrust*(1-(1/(double)stage.at(currentStage).nEngines));

		int engineout = rand() % stage.at(currentStage).nEngines;
		//SetThrusterMax0(stage.at(currentStage).th_main_h.at(engineout),newlevel);

		//SetThrusterLevel(stage.at(currentStage).th_main_h.at(engineout),0);


		SetThrusterResource(stage.at(currentStage).th_main_h.at(engineout), NULL);

		//	DelExhaust(exhaustN[currentStage][engineout]);
		//sprintf(logbuff,"engineout:%i exhaustN:%i oldlevel %.1f newlevel %.1f",engineout,exhaustN[currentStage][engineout],stage.at(currentStage).thrust,newlevel);
		//oapiWriteLog(logbuff);
	}
}
...
void Multistage2015::clbkLoadStateEx(FILEHANDLE scn, void* vs)
{
	...
	//sprintf(logbuff,"currentInterstage:%i currentStage:%i csIncremental:%i nInterstages:%i",currentInterstage,currentStage,stage.at(currentStage).IntIncremental,nInterstages);
	//oapiWriteLog(logbuff);
	if ((currentInterstage > currentStage) || (currentInterstage > nInterstages) || (currentInterstage >= stage.at(currentStage).IntIncremental)) { stage.at(currentStage).wInter = false; }
...
void Multistage2015::clbkSaveState(FILEHANDLE scn)
{
	...
	if (stage.at(currentStage).batteries.wBatts)
	{
		...
		sprintf(savevalbuff, "%lf", stage.at(currentStage).batteries.CurrentCharge);
		oapiWriteScenario_string(scn, savebuff, savevalbuff);
	}
	...
	sprintf(savevalbuff, "%i", stage.at(currentStage).StageState);
	oapiWriteScenario_string(scn, savebuff, savevalbuff);
...
void Multistage2015::CreateCamera()
{
	...
	if (!oapiIsVessel(hcamera)) {
		...
		{
			VESSEL3* vcam;
			vcam = (VESSEL3*)oapiGetVesselInterface(hcamera);
