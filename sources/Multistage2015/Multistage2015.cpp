...
void Multistage2015::VehicleSetup() {
	...
	nPsh = 0;
	for (unsigned int pp = 0; pp < nStages; pp++)
	{
		for (unsigned int ppp = 0; ppp < stage[pp].nEngines; ppp++)
		{
			nPsh++;
		}
	}
	for (unsigned int pb = 0; pb < nBoosters; pb++)
	{
		for (unsigned int ppb = 0; ppb < booster[pb].N; ppb++)
		{
			for (unsigned int ppbb = 0; ppbb < booster[pb].nEngines; ppbb++)
			{
				nPsh++;
			}
		}
	...
	for (unsigned int ps = 0; ps < nPsh; ps++)
	{
...
void Multistage2015::FLY(double simtime, double simdtime, double mjdate) {
	...
	if (wBoosters) {
		for (int nb = 0; nb < nBoosters; nb++) {
			if (booster[nb].currDelay > 0) {
				booster[nb].currDelay -= simdtime;
			}
		}
	}
